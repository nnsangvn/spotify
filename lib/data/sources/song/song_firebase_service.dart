...
class SongFirebaseServiceImpl extends SongFirebaseService {
  ...
  Future<Either> getUserFavoriteSongs() async {
    try {
      ...
      QuerySnapshot favoriatesSnapshot =
          await firebaseFirestore.collection('Users').doc(uId).collection('Favorites').get();

      for (var element in favoriatesSnapshot.docs) {
        String songId = element['songId'];
        var song = await firebaseFirestore.collection('Songs').doc(songId).get();
        SongModel songModel = SongModel.fromJson(song.data()!);
        songModel.isFavorite = true;
        songModel.songId = songId;
        favoriateSongs.add(songModel.toEntity());
      }
      return Right(favoriateSongs);
    } catch (e) {
      print(e);
